/*
 * Auto-generated UAPI header for m_foo
 * Author: Kevin
 * Date: 
 * Description: User-Space API (UAPI) contract for m_foo.
 */

#ifndef _UAPI_LINUX_M_FOO_H_
#define _UAPI_LINUX_M_FOO_H_
...
#define M_FOO_IOC_RESET       _IO(M_FOO_IOC_MAGIC, 0)

/* Command 1: Write a configuration struct TO the driver */
#define M_FOO_IOC_SET_CONFIG  _IOW(M_FOO_IOC_MAGIC, 1, struct m_foo_config)

/* Command 2: Read a data struct FROM the driver */
#define M_FOO_IOC_GET_DATA    _IOR(M_FOO_IOC_MAGIC, 2, struct m_foo_data)

/* Command 3: Send a simple 32-bit integer and get a status integer back */
#define M_FOO_IOC_CALIBRATE   _IOWR(M_FOO_IOC_MAGIC, 3, __u32)
