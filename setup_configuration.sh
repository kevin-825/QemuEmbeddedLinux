...
load_config() {
    ...
    # We pass the full path so Buildroot knows exactly where to find it
    eval $DOCKER_CMD make -C "$BUILDROOT_DIR" O="$OUTPUT_DIR" BR2_EXTERNAL="$EXTERNAL_DIR" "$config_path"
