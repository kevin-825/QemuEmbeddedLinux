/*
 * Auto-generated UAPI header for helloMod
 * Author: Kevin
 * Date: 
 * Description: User-Space API (UAPI) contract for helloMod.
 */

#ifndef _UAPI_LINUX_HELLOMOD_H_
#define _UAPI_LINUX_HELLOMOD_H_
...
struct helloMod_config {
    __u32 sample_rate;
    __u32 active_channels;
};

struct helloMod_data {
...
#define HELLOMOD_IOC_RESET       _IO(HELLOMOD_IOC_MAGIC, 0)

/* Command 1: Write a configuration struct TO the driver */
#define HELLOMOD_IOC_SET_CONFIG  _IOW(HELLOMOD_IOC_MAGIC, 1, struct helloMod_config)

/* Command 2: Read a data struct FROM the driver */
#define HELLOMOD_IOC_GET_DATA    _IOR(HELLOMOD_IOC_MAGIC, 2, struct helloMod_data)

/* Command 3: Send a simple 32-bit integer and get a status integer back */
#define HELLOMOD_IOC_CALIBRATE   _IOWR(HELLOMOD_IOC_MAGIC, 3, __u32)
