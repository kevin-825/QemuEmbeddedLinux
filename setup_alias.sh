...
#alias run_in_docker='docker run -it -e TERM=${TERM} --rm ${dockerVolumes} -w $(pwd -P) kflyn825/rvdev:latest bash -c'
...
run_rv_virt() {
    ...
    -display none \
    -serial stdio \
    -kernel /mnt/wsl/ramdisk5/kbuild_out_t0/riscv/arch/riscv/boot/Image \
    -append "console=ttyS0 root=/dev/vda ro earlycon" \
    -drive file=/mnt/wsl/ramdisk5/out/qemu_riscv64_virt_board/customBuild/images/rootfs.ext4,format=raw,id=hd0,if=none \
    -device virtio-blk-device,drive=hd0
}