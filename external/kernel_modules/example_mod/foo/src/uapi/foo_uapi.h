/*
 * Auto-generated UAPI header for foo
 * Author: Kevin
 * Date: 
 * Description: User-Space API (UAPI) contract for foo.
 */

#ifndef _UAPI_LINUX_FOO_H_
#define _UAPI_LINUX_FOO_H_
...
#define FOO_IOC_SET_CONFIG  _IOW(FOO_IOC_MAGIC, 1, struct foo_config)

/* Command 2: Read a data struct FROM the driver */
#define FOO_IOC_GET_DATA    _IOR(FOO_IOC_MAGIC, 2, struct foo_data)

/* Command 3: Send a simple 32-bit integer and get a status integer back */
#define FOO_IOC_CALIBRATE   _IOWR(FOO_IOC_MAGIC, 3, __u32)
